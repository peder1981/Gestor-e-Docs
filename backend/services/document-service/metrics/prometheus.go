...
var (
	...
	HTTPDuration = prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "document_service_http_duration_seconds",
	...
	// DocumentOperations conta operações de documentos por tipo
	DocumentOperations = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "document_service_operations_total",
			Help: "Total de operações em documentos por tipo.",
		},
		[]string{"operation"},
	)

	// MinIOOperations conta operações no MinIO por tipo
	MinIOOperations = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "document_service_minio_operations_total",
			Help: "Total de operações no MinIO por tipo.",
		},
	...
	MongoDBOperations = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "document_service_mongodb_operations_total",
...
func Init() {
	...
	prometheus.MustRegister(MinIOOperations)
